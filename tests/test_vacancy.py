...
def test_str(some_vacancy):
    assert (
        some_vacancy.__str__() == "ТЕСТОВАЯ ВАКАНСИЯ\n"
        "*****************\n"
        "Работодатель: Тестовый работодатель\n"
        "Зарплата: 100000 - 150000 RUB\n"
        "Адрес: Г.Тестовый, ул Тестовая, 1\n"
        "Обязанности: Тестовый текст обязанностей в вакансии\n"
        "Ссылка: https://test.ru"
    )


def test_str_without_salary_from(some_vacancy_without_salary_from):

    assert (
        some_vacancy_without_salary_from.__str__() == "ТЕСТОВАЯ ВАКАНСИЯ\n"
        "*****************\n"
        "Работодатель: Тестовый работодатель\n"
        "Зарплата: до 190000 RUB\n"
        "Адрес: Г.Тестовый, ул Тестовая, 2\n"
        "Обязанности: Тестовый текст обязанностей в вакансии номер 3\n"
        "Ссылка: https://test.ru"
    )


def test_str_without_salary_to(some_vacancy_without_salary_to):

    assert (
        some_vacancy_without_salary_to.__str__() == "ТЕСТОВАЯ ВАКАНСИЯ\n"
        "*****************\n"
        "Работодатель: Тестовый работодатель\n"
        "Зарплата: от 140000 RUB\n"
        "Адрес: Г.Тестовый, ул Тестовая, 2\n"
        "Обязанности: Тестовый текст обязанностей в вакансии номер 3\n"
        "Ссылка: https://test.ru"
    )
